...
class LoggingMiddleware:
    ...
    def _is_redundant_repeat(self, method: str, path: str, status_code: int, now: float) -> bool:
        ...
        non-2xx are never deduped. Stamps only on emit, so steady polls log."""
        if _ACCESS_LOG_DEDUP_MS <= 0:
            return False
        if method != "GET" or not (200 <= status_code < 300):
            return False
        key = (method, path, status_code)
        last = self._last_log.get(key)
        if last is not None and (now - last) * 1000.0 < _ACCESS_LOG_DEDUP_MS:
            return True
        self._last_log[key] = now
        if len(self._last_log) > _DEDUP_MAP_MAX:
            cutoff = now - (_ACCESS_LOG_DEDUP_MS / 1000.0)
